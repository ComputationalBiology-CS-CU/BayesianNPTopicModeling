...
int main() {
    ...
    double v_alpha [S] [J] [2];
    double v_omega [K] [2];
    double v_gamma [K] [N] [T] [2];
    double v_eta [K] [M] [3];
    double phiE [S] [N] [J];
    double phiO [S] [M] [J];
    double v_nu [S] [J] [K];
    ...
    while (true) {
        ...
        for (int s = 0; s < S; s++) {
            for (int n = 0; n < N; n++) {
                ...
                for (int j = 0; j < J; j++) {
                    ...
                    if (j > 0) {
                        dialpha_sum += digamma(v_alpha [s][j-1][1], &ifault) - digamma(v_alpha [s][j-1][0] + v_alpha [s][j-1][1] , &ifault);
                    }
                    for (int t = 0; t < T; t++) {
                        for (int k = 0; k < K; k++) {
                            phiE [s][n][j] += v_nu [s][j][k] * (E [s][n][t] * ( digamma(v_gamma [k][n][t][0], &ifault) - log(v_gamma [k][n][t][1]) ) - v_gamma [k][n][t][0]/v_gamma [k][n][t][1] - stirling_approx(E [s][n][t]));
                        }
                    }
                    phiE [s][n][j] += digamma(v_alpha [s][j][0], &ifault) - digamma(v_alpha [s][j][0] + v_alpha [s][j][1], &ifault) + dialpha_sum;
                }
            }
        }
        //  softmax normalize
        ...
        for (int s = 0; s < S; s++) {
            for (int m = 0; m < M; m++) {
                ...
                for (int j = 0; j < J; j++) {
                    ...
                    for (int l = 0; l < 3; l++) {
                        for (int k = 0; k < K; k++) {
                            phiO [s][m][j] += O_onehot [s][m][l] * v_nu [s][j][k] * ( digamma(v_eta [k][m][l], &ifault) - digamma(v_eta [k][m][0] + v_eta [k][m][1] + v_eta [k][m][2], &ifault) );
                        }
                    }
                    phiO [s][m][j] += digamma(v_alpha [s][j][0], &ifault) - digamma(v_alpha [s][j][0] + v_alpha [s][j][1], &ifault) + dialpha_sum;
                }
            }
        }
        //  softmax normalize
        ...
        for (int s = 0; s < S; s++) {
            for (int j = 0; j < J; j++) {
                ...
                for (int k = 0; k < K; k++) {
                    v_nu [s][j][k] = 0;
                    if (k > 0) {
                        diomega_sum += digamma(v_omega [k-1][1], &ifault) - digamma(v_omega [k-1][0] + v_omega [k-1][1] , &ifault);
                    }
                    for (int m = 0; m < M; m++) {
                        for (int l = 0; l < 3; l++) {
                            v_nu [s][j][k] += O_onehot [s][m][l] * phiO [s][m][j] * ( digamma(v_eta [k][m][l], &ifault) - digamma(v_eta [k][m][0] + v_eta [k][m][1] + v_eta [k][m][2], &ifault) );
                        }
                    }
                    for (int n = 0; n < N; n++) {
                        for (int t = 0; t < T; t++) {
                            v_nu [s][j][k] += phiE [s][n][j] * (E [s][n][t] * ( digamma(v_gamma [k][n][t][0], &ifault) - log(v_gamma [k][n][t][1]) ) - v_gamma [k][n][t][0]/v_gamma [k][n][t][1] - stirling_approx(E [s][n][t]));
                        }
                    }
                    v_nu [s][j][k] += digamma(v_omega [k][0], &ifault) - digamma(v_omega [k][0] + v_omega [k][1], &ifault) + diomega_sum;
                }
            }
        }
        //  softmax normalize
        ...
        for (int k = 0; k < K; k++) {
            ELBO_kk += (omega - v_omega [k][1]) * (digamma(v_omega [k][1], &ifault) - digamma(v_omega [k][0] + v_omega [k][1], &ifault)) + (1-v_omega [k][0]) * (digamma(v_omega [k][0], &ifault) - digamma(v_omega [k][0] + v_omega [k][1], &ifault)) - log(omega) + log_beta(v_omega [k][0], v_omega [k][1]);
            
            for (int m = 0; m < M; m++) {
                double di_eta = digamma(v_eta [k][m][0] + v_eta [k][m][1] + v_eta [k][m][2], &ifault);
                for (int l = 0; l < 3; l++) {
                    ELBO_km += (eta [l] - v_eta [k][m][l]) * (digamma(v_eta [k][m][l], &ifault) - di_eta);
                }
            ...
            for (int n = 0; n < N; n++) {
                for (int t = 0; t < T; t++) {
                    ELBO_knt += (gamma - v_gamma [k][n][t][0]) * (digamma(v_gamma [k][n][t][0], &ifault) - log(v_gamma [k][n][t][1])) - v_gamma [k][n][t][0]/v_gamma [k][n][t][1] + v_gamma [k][n][t][0] - v_gamma [k][n][t][0] * log(v_gamma [k][n][t][1]) + stirling_approx(v_gamma [k][n][t][0]-1) - stirling_approx(gamma-1);
                }
            }
        }
        ...
        for (int k = 0; k < K; k++) {
            if (k > 0) {
                diomega_i += digamma(v_omega [k-1][1], &ifault) - digamma(v_omega [k-1][0] + v_omega [k-1][1], &ifault);
            }
            double diomega_k = digamma(v_omega [k][0], &ifault) - digamma(v_omega [k][0] + v_omega [k][1], &ifault) + diomega_i;
            for (int s = 0; s < S; s++) {
                for (int j = 0; j < J; j++) {
        ...
        for (int s = 0; s < S; s++) {
            for (int j = 0; j < J; j++) {
                ELBO_sj += (alpha - v_alpha [s][j][1]) * ( digamma(v_alpha [s][j][1], &ifault) - digamma(v_alpha [s][j][0] + v_alpha [s][j][1], &ifault) ) - (v_alpha [s][j][0] - 1) * (digamma(v_alpha [s][j][0], &ifault) - digamma(v_alpha [s][j][0] + v_alpha [s][j][1], &ifault)) + log_beta(v_alpha [s][j][0], v_alpha [s][j][1]);
            }
        }
        ...
        for (int s = 0; s < S; s++) {
            ...
            for (int j = 0; j < J; j++) {
                if (j > 0) {
                    dialpha_i += digamma(v_alpha [s][j-1][1], &ifault) - digamma(v_alpha [s][j-1][0] + v_alpha [s][j-1][1], &ifault);
                }
                double dialpha_sj = digamma(v_alpha [s][j][0], &ifault) - digamma(v_alpha [s][j][0] + v_alpha [s][j][1], &ifault) + dialpha_i;
                for (int m = 0; m < M; m++) {
                    ...
                    for (int k = 0; k < K; k++) {
                        double di_eta = digamma(v_eta [k][m][0] + v_eta [k][m][1] + v_eta [k][m][2], &ifault);
                        for (int l = 0; l < 3; l++) {
                            ELBO_sml += v_nu [s][j][k] * phiO [s][m][j] * O_onehot [s][m][l] * ( digamma(v_eta [k][m][l], &ifault) - di_eta );
                        }
                    }
                }
                for (int n = 0; n < N; n++) {
                    ...
                    for (int k = 0; k < K; k++) {
                        for (int t = 0; t < T; t++) {
                            ELBO_snt += v_nu [s][j][k] * phiE [s][n][j] * ( E [s][n][t] * (digamma(v_gamma [k][n][t][0], &ifault) - log(v_gamma [k][n][t][1])) - v_gamma [k][n][t][0]/v_gamma [k][n][t][1] - stirling_approx(E [s][n][t]) );
                        }
                    }
                }
            }
